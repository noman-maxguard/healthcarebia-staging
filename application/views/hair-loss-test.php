<!doctype html>
<html lang="en">
<head>
<script type="application/ld+json">
{
  "@context":"https://schema.org",
  "@type":"MedicalTest",
  "name":"Hair Loss Test",
  "description":"Comprehensive blood panel assessing common contributors to hair loss.",
  "url":"https://www.healthcarebia.ae/hair-loss-test",
  "isAccessibleForFree": false,
  "availableService": { "@type":"Service", "areaServed": { "@type":"City", "name":"Dubai" } },
  "offers": { "@type":"Offer", "priceCurrency":"AED", "url":"https://www.healthcarebia.ae/hair-loss-test" },
  "provider": { "@type":"MedicalOrganization", "name":"Healthcarebia", "url":"https://www.healthcarebia.ae" }
}
</script>


    <?php include 'includes/inc_head_tag.php'; ?>


</head>
<body>
<?php include 'includes/inc_header.php'; ?>


<section class="sub-banner sub-banner-3"
         style="background-image: url(<?= base_url() ?>assets/frontend/img/sub-banner-3.webp);">
    <div class="overlay">
        <div class="container">
            <div class="row">
                <div class="col-12"><h1>Hair Loss Test</h1>
                    <nav aria-label="breadcrumb">
                        <ul class="breadcrumb">
                            <li><a href="<?= base_url() ?>" class="hvr-underline-from-left menu-line">Home</a></li>
                            <li><a class="hvr-underline-from-left menu-line">General Health</a></li>
                            <li class="active" aria-current="page">Hair Loss Test</li>
                        </ul>
                    </nav>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="section-gap">
    <div class="container">
        <div class="row">
            <div class="col-md-5 mb-3"><img src="<?= base_url() ?>assets/frontend/img/hair-loss.webp" alt="Hair Loss Test Dubai" width="497" height="490"
                                            class="img-fluid rounded "></div>
            <div class="col-md-7">
                <div class="inner-page-right">
                    <div class="mt-4"><p>The Hair Loss Test offers insights into potential factors contributing to hair
                            loss. It examines genetics, lifestyle, and health, providing a comprehensive understanding.
                            This assessment aids in making informed decisions about hair care. Remember, hair health can
                            reflect overall well-being.</p>
                        <ul class="listing-item3">
                            <li><img src="<?= base_url() ?>assets/frontend/img/biomarkers.svg" width="30" height="30" alt="Hair Loss Test Dubai icon 1">8 Biomarkers
                                tested
                            </li>
                            <li><img src="<?= base_url() ?>assets/frontend/img/hormones.svg" width="30" height="30" alt="Hair Loss Test Dubai icon 2"> Hormones,
                                Cholesterol, Blood Sugar & More
                            </li>
                            <li><img src="<?= base_url() ?>assets/frontend/img/home.svg" width="30" height="30" alt="Hair Loss Test Dubai icon 3">At home blood sample
                                collection
                            </li>
                            <li><img src="<?= base_url() ?>assets/frontend/img/dha.svg" width="30" height="30" alt="Hair Loss Test Dubai icon 4">DHA-licensed nurses & lab
                                partners
                            </li>
                            <!-- <li><img src="<?= base_url() ?>assets/frontend/img/12-hour.svg" alt="Hair Loss Test Dubai icon 1">Results in as little
                                as 12 hours.
                            </li> -->
                            <li><img src="<?= base_url() ?>assets/frontend/img/results-icon.svg" width="30" height="30" alt="Hair Loss Test Dubai icon 5">Fast, secure,
                                and confidential results.
                            </li>
                        </ul>
                    </div>
                    <div class="mt-4"><span class="price-tag">AED 500</span></div>
                    <a href="<?= $whatsappHref ?>" class="primary-btn hvr-bounce-to-right green-btn book-now mt-4">Book your Appointment with us now</a>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="tabs-block section-gap">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="tabs-design">
                    <nav>
                        <div class="nav nav-tabs mb-3" id="nav-tab" role="tablist">
                            <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab"
                                    data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home"
                                    aria-selected="true"> Markers Tested
                            </button>
                            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab"
                                    data-bs-target="#nav-profile" type="button" role="tab" aria-controls="nav-profile"
                                    aria-selected="false"> Related Symptoms
                            </button>
                            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab"
                                    data-bs-target="#nav-why-healthcarebia" type="button" role="tab"
                                    aria-controls="nav-profile" aria-selected="false"> Why Healthcarebia
                            </button>
                        </div>
                    </nav>
                    <div class="tab-content" id="nav-tabContent">
                        <div class="tab-pane fade active show" id="nav-home" role="tabpanel"
                             aria-labelledby="nav-home-tab">
                            <ul class="listing-item4">
                                <li>Complete Blood Count</li>
                                <li>Ferritin in Serum</li>
                                <li>Iron in Serum</li>
                                <li>DHEA-S</li>
                                <li>Vitamin B12</li>
                                <li>Vitamin D</li>
                                <li>Zinc in Plasma</li>
                                <li>TSH</li>
                            </ul>
                        </div>
                        <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                            <ul class="listing-item4">
                                <li>Duration and Severity of Hair Loss</li>
                                <li>Stressed individuals</li>
                                <li>Suffering hair, skin, and nail health decline</li>
                                <li>Family or Personal History</li>
                                <li>Diet and Nutrition Changes</li>
                                <li>Medications</li>
                            </ul>
                        </div>
                        <div class="tab-pane fade" id="nav-why-healthcarebia" role="tabpanel"
                             aria-labelledby="nav-profile-tab">
                            <ul class="listing-item5">
                                <li>First in the MENA region to achieve the ISO 15189-2012 accreditation.</li>
                                <li>State of the art incorporation of technology and equipment to ensure maximum
                                    efficiency, accuracy, and quality of results.
                                </li>
                                <li>Customized end to end laboratory information system, co-developed with expert
                                    Pathologists.
                                </li>
                                <li>Global Investigation Laboratories from around the world.</li>
                                <li>Incorporating Next Gen Technologies – Genomics, Proteomics, Metabolomics, Molecular
                                    Biology, Digital Pathology.
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="faq-block section-gap" style="background-color: #ffffff;">
    <div class="container">
        <h2>FAQ's</h2>
        <div class="row">
            <div class="col-md-6">
                <div class="accordion pt-md-3">

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>1</span>What is the Hair Loss Test?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>The Hair Loss Test is an assessment designed to explore possible factors that may be contributing to hair thinning or hair fall. It looks beyond the surface, bringing together information about your genetics, lifestyle and general health so that you and your healthcare professional can have a more informed conversation about your hair concerns.</p>
                        </div>
                    </div>

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>2</span>What does the Hair Loss Test usually assess?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>This test considers several areas that may be linked to hair loss. These can include inherited or family patterns of hair thinning, lifestyle factors such as stress, sleep and nutrition, and aspects of your overall health. Together, these insights help build a more complete picture of how your daily life and wellbeing may be reflected in your hair health.</p>
                        </div>
                    </div>

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>3</span>Who should consider doing the Hair Loss Test?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>This assessment may be helpful for people who have noticed ongoing hair thinning, widening of the part line, increased shedding or patchy areas of hair loss. It can also support anyone who is concerned about how their lifestyle, stress levels or general health might be affecting their hair and would like structured guidance rather than guessing or trying random treatments.</p>
                        </div>
                    </div>

                </div>
            </div>

            <div class="col-md-6">
                <div class="accordion pt-md-3">

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>4</span>Does this test diagnose the exact cause of my hair loss?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>No. The Hair Loss Test does not on its own diagnose a specific condition or guarantee an explanation for every case of hair loss. Instead, it offers additional information about factors that may be relevant. A qualified clinician will review your results together with your medical history, physical examination and other tests before making any diagnosis or recommending treatment.</p>
                        </div>
                    </div>

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>5</span>How is the Hair Loss Test usually carried out?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>Depending on the specific package, the assessment may include a detailed questionnaire about your lifestyle, family history and symptoms, along with laboratory analysis such as blood tests or other samples. Instructions are provided in advance so you know how to prepare and what to expect, and the process is designed to be as straightforward and comfortable as possible.</p>
                        </div>
                    </div>

                    <div class="accordion-box">
                        <div class="accordion-item">
                            <h3><span>6</span>What should I do with my Hair Loss Test results?</h3>
                            <span class="plus-minus"></span>
                        </div>
                        <div class="data" style="display: none;">
                            <p>Your results are best used as a starting point for a guided discussion with a healthcare professional. They can help you interpret the findings in context, link them to your symptoms and overall wellbeing, and advise on next steps for hair care, lifestyle changes or further medical evaluation rather than making decisions on your own.</p>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>
</section>

<?php include 'includes/inc_footer.php'; ?>



<?php include 'includes/inc_footer_scripts.php'; ?>
</body>
</html>